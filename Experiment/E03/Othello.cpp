...
{
	Othello board;                   		//初始化棋盘 
	Othello *pBoard = &board;				//方便复制棋盘的操作 
...
start:						
	...
	cout << ">>>人机对战开始： \n";
		...
		while (num<36)													// 棋盘上未下满36子 
		{
			...
			{
				int i, j;
				board.Show(pBoard);                  //显示棋盘 

				if (present == player)        		//玩家下棋 
				{
					while (1)
					{
				...
				{   
					...
					pChoice = Find(pBoard, present, deepth, -MAX, MAX, pChoice);     //寻找最合适的落棋点 
					i = pChoice->pos.first;
				...
				board.Action(pBoard, pChoice, present);       //落子,并修改棋盘
				num++;
...
void Othello::Create(Othello *board)
{
	...
	board->cell[2][2].color = board->cell[3][3].color = WHITE;    //初始化棋盘棋子颜色 
